...
- Numeric layout authoring must match Core's integer grid domains and the runtime utility classes. Test fractional, non-finite, out-of-range, and inherited values through the column editor and JSON apply path. Runtime recovery must not silently rewrite the authored document; invalid drafts must block Apply/Save and identify the field and breakpoint. Revalidate JSON at Apply time, not only after a debounced edit.
- Row occupancy is authoring guidance, not a requirement to fill 12 units. Include responsive offsets, exclude breakpoint-hidden columns and columns with only statically hidden fields, and preserve intentional whitespace. Empty draft columns and visual blocks still participate. Suggestions must remain explicit, use the same occupancy calculation and never rewrite metadata or layout on inspection. Do not present this static guidance as a simulation of record-dependent rules.
- Visibility commands must preserve geometry, ordering and style. Before changing smaller breakpoints, resolve inheritance so that an action scoped to them does not unintentionally hide larger breakpoints. Test presets against a customized column, not only default full-width fixtures.
...
- For individual field movement, expose and validate the existing selected-field **Move field** dialog as a keyboard-accessible alternative to drag. Choose section/row/column and position through `moveLayoutItemById`; preserve item IDs, metadata, mixed-item order and empty containers. The destination column owns geometry/style. Opening/cancelling must not dirty the document; reject an obsolete destination snapshot and move focus to the selected field after confirmation. Validate Apply → Save/close → Save page → reload separately, then restore the baseline when simulating on a user composition.
- Destination summaries must derive from canonical `items[]` and metadata labels, preserve stable IDs as option values, account for visual blocks/empty destinations, and bound long lists without treating static summaries as visibility/permission simulation.
- For visual-block destinations, reuse the internal `visual-block-label.ts` extraction shared with the row editor. Bound plain-text previews and distinguish identical/empty blocks by original column item position (including the moving field); never use preview text or position as persisted identity, evaluate bindings/HTML, or infer a preset from CSS classes. Validate duplicate text with distinct IDs and multiline options in a narrow dialog.
- Visual-block preview shortcuts must handle Enter/Space only when the preview itself is the event target. Nested native buttons own their activation: do not open the content editor or prevent Space on their behalf. Validate menu open/remove by keyboard, preview edit/select shortcuts, unchanged document on inspection, and add/remove returning to a clean draft. After insertion/reorder/column movement, continue focus on the selected block; after removal, select/focus its preserved column. Guard deferred focus against a newer selection. Test through a parent that clones emitted config: object-reference equality alone must not suppress valid focus restoration.
- Scope `DynamicFormLayoutService` to each `LayoutEditorComponent` draft: root `changes$` is observed by live forms, so using that root instance for draft operations bypasses Apply and leaks cancelled changes into runtime. Validate both same-page discard/reopen and explicit Apply; a reload or an unchanged HTTP payload alone cannot prove draft isolation.
- Section quick rename must reuse `FormSection.title` and the existing update event, preserve IDs/structure/appearance, reject blank names, and support Enter/Escape with focus return. Empty-section guidance should invoke the existing add-row command with translated copy and host tokens. When validating section removal, check that eligible unplaced API fields return to the palette without deleting metadata, then discard/reopen on the same page; visual blocks belong to the removed structure and are not recoverable API fields. Tests of output-driven operations must reproduce the owning editor's output-to-input binding.
- Section spacing is `FormSection.gapBottom`, the space after the whole section, not a column gap or editor density. Inspect `LayoutEditorComponent.canApplyAll/applyGapToAll` and `docs/section-spacing-authoring-2026-09-08.md`: fill only absent/null values, preserve explicit zero and other authored gaps, reject negative/non-finite proposals, and emit nothing for a proposal change or an ineligible document. Do not generate or consult `gapCustomized` as a second source of eligibility. Validate through the owning output-to-input flow, including same-page discard/reopen; a draft-only check does not prove persisted runtime spacing.
...
- Audit the existing column-menu Add API field path before inventing another placement contract. Explain it in palette/empty-column copy, validate Enter/arrow navigation, and focus the inserted field after rendering: the old menu trigger may be destroyed by the layout update. Tab must continue into the field controls; reject already placed/unknown candidates and verify discard/reopen returns the field to the available catalogue.

...
- A public Angular catalog must reuse the installed release's package manifests and component registries. Do not infer exports, import paths, or runtime component availability from a registry entry, and preserve explicit `contextOnly` and `contractOnly` entries as semantic contexts and runtime contracts, distinct from catalogued component surfaces. Carry each package manifest status into compact LLM projections as well as full catalogs.

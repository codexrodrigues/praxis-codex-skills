...
- When a documentation projector consumes installed `@praxisui/dynamic-fields` component-registry metadata, resolve `friendlyName` and `description` values that are i18n keys through the public canonical dictionary from the same pinned release (`PRAXIS_DYNAMIC_FIELDS_PT_BR`), or fail with the unresolved key. Do not copy translations into the host or create a second translation runtime.
- A registry name or ID is discovery metadata, not an import instruction and not proof that the component has runtime integration. Preserve that boundary in public catalog copy and validation.

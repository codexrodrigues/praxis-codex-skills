...
- A new submit/edit/resend must clear abandoned quick replies, clarification questions, review authorization, preview, pending patch, pending clarification, errors, diagnostics, observation ID, and any prior active semantic decision before the next result materializes.
...
- Treat `contextHints.semanticDecision` as grounding evidence for the backend, not as an active route decision. If a quick reply has only hint-level semantic evidence, forward it inside structured `contextHints` but keep `activeSemanticDecision` unset; the next free prompt must not inherit it.
...
- Surface lifecycle evidence such as probe readiness, transport opening, and first event as non-terminal diagnostics. If the first event is a heartbeat, `firstEventSeq=-1` is a liveness marker, not replay position. Apply silence, result, and whole-turn timeouts as recoverable transport failures; do not fabricate a backend result.

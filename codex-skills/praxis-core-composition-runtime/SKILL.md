...
- The stable transform catalog executes `identity`, `constant`, `pick-path`, `template`, `object-template`, `array-template`, `coalesce`, `merge-objects`, `select-case`, and `format-value`. Inspect the current catalog before relying on other `TransformKind` members; a declared union member alone is not execution evidence. Pipeline and derived-state formatting share the Core formatter, with explicit configuration rather than inference from field names. Read [selection-presentation.md](references/selection-presentation.md) for date/mask semantics, coherent transient identity, editor proof, and the agentic shorthand boundary.
...
- Missing composition runtime, invalid dispatch source, unmatched links, or blocked delivery are structured runtime outcomes. Do not recover by guessing another source endpoint, parsing `surface.result.type`, or writing state directly in the host; repair the authored link/source, provider bootstrap, or composition definition and surface the diagnostic.

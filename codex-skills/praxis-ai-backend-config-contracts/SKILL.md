...
- Treat `streamAuthMode` and `streamAccessToken` from the generated start response as backend-issued transport authority for that stream only. The Angular client may forward the returned token to probe/connect/cancel exactly as the contract permits, but must not mint tokens, cache them across turns, reuse them across streams, derive them from session/client turn IDs, or downgrade required host headers into query parameters.
...
- Decision diagnostics such as `semanticDecisionReviewGroundedByPreview`, schema verification, lexical fallback flags, and tool-loop traces are audit evidence. They may explain why a result needs review or is unsafe, but cannot grant `canApply`, create an apply payload, or override backend risk confirmation policy in the browser.

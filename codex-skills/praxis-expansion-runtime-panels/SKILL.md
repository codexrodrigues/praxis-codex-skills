...
- Use `praxis-navigation-container-composition-events` for panel-aware nested widget events, lazy panel content, action button boundaries, dynamic page embedding, provider metadata, and composition links.
- Use `praxis-navigation-agentic-registry` for expansion AI adapters, agentic turn flows, context packs, registry projection, and cross-container authoring consistency.

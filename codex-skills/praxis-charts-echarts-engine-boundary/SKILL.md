...
Validate theme changes against both the engine canvas and the rendered HTML surface, not only text tokens or the outer widget. In `applyInheritedTheme`, an explicitly embedded/transparent chart must not inherit the page preset's contained background while taking the host foreground. Preserve the inherited palette and authored surface/background/text overrides. Test a dark/light/dark host transition with the executive page preset, the actual shell computed background, and an unchanged input document. Inspect `widget-chart-theme.playwright.spec.ts` in the Dynamic Page Lab for the real consumer proof. Axis-name mapping alone does not prove that foreground and surface provenance agree; explicit custom colors still require a coherent pair.

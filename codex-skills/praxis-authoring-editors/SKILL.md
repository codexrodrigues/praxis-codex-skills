...
For `@praxisui/crud`, use `praxis-crud-runtime-openmodes` for runtime/open-mode/launcher behavior, `praxis-crud-drawer-adapter-contract` for drawer adapter entrypoint and host drawer parity, `praxis-crud-dialog-form-host-lifecycle` for modal/drawer dynamic-form lifecycle and refresh, and `praxis-crud-ai-authoring` for CRUD authoring manifest, config editor, and child-operation delegation. For `@praxisui/dialog`, use `praxis-dialog-overlay-runtime` for overlay/focus/preset behavior, `praxis-dialog-global-actions-ai` for global actions, registries, and manifest work, and `praxis-dialog-surface-global-actions` for surface materialization and drawer bridge behavior.

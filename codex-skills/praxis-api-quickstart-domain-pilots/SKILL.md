...
When a pilot demonstrates shadow or dual-run operation, keep it host-owned and
strictly non-authoritative. The baseline must be independent of the candidate:
it cannot copy the candidate result, read its snapshot payload, or use a
persisted candidate outcome as its oracle. Freeze facts, `nowUtc`, timezone and
resolved actor context once, then run both sides with isolated, bounded timeout,
concurrency and saturation behavior. Classify `MATCH`, `MISMATCH`,
`INCONCLUSIVE` and `TECHNICAL_ERROR` without converting failures into business
denials. A shadow path must not use mutation/idempotency ledgers, advance a
lifecycle, skip the approved baseline route, or execute an EffectIntent. Publish
only allowlisted observations and low-cardinality metrics; never tag or log
facts, request references, actors, amounts, exception messages, secrets, or

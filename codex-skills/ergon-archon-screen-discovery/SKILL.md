...
When available, consult the derived `praxis-screen-discovery-matrix` before
starting discovery to choose comparable pilots and identify known contracts or
runtime gaps. Regenerate it after discovery changes the operation inventory or
phase gate. It is an index only: XML/runtime evidence, the phase gate and the

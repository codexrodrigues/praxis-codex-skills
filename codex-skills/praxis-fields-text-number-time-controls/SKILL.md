...
- Composite range controls must expose distinct localized accessible names for their start and end inputs. Derive fallbacks from the owning field label, preserve meaningful explicit start/end overrides, and verify the actual Material input DOM after its render lifecycle; a projected attribute before Material normalization is not sufficient evidence.
...
- For range accessible-name changes, add a real Dynamic Form or Table consumer check so both subcontrols retain distinct names after projection, overlay opening and locale resolution.

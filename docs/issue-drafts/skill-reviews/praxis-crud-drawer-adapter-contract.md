# Revisar skill praxis: praxis-crud-drawer-adapter-contract

## Objetivo

Solicitar a um especialista em plataforma Praxis uma revisao criteriosa da skill praxis-crud-drawer-adapter-contract para garantir que ela esteja 100% em conformidade com os recursos disponiveis do Praxis e com as melhores praticas de uso desses recursos.
...
- Familia: praxis
- Caminho: codex-skills/praxis-crud-drawer-adapter-contract/
- Descricao atual: Use when changing or reviewing `@praxisui/crud` drawer mode, `CRUD_DRAWER_ADAPTER`, `CrudDrawerAdapter`, the lightweight `@praxisui/crud/drawer-adapter` secondary entrypoint, default drawer fallback, drawer result callbacks, bundle-entrypoint ADR alignment, or host drawer integration divergence.

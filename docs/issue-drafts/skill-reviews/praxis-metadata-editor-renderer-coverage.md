# Revisar skill praxis: praxis-metadata-editor-renderer-coverage

## Objetivo

Solicitar a um especialista em plataforma Praxis uma revisao criteriosa da skill praxis-metadata-editor-renderer-coverage para garantir que ela esteja 100% em conformidade com os recursos disponiveis do Praxis e com as melhores praticas de uso desses recursos.
...
- Familia: praxis
- Caminho: codex-skills/praxis-metadata-editor-renderer-coverage/
- Descricao atual: Use when implementing, reviewing, or auditing `@praxisui/metadata-editor` renderer coverage: `FieldMetadataEditorComponent`, `DynamicEditorRendererComponent`, `EditorProperty[]` configs, inline editor coverage, dynamic-fields parity, form factory controls, visual coverage versus JSON-only fallback, hints, i18n labels, and public API for metadata editor components/services.

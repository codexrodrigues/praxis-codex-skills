# Revisar skill praxis: praxis-manual-form-autosave-persistence

## Objetivo

Solicitar a um especialista em plataforma Praxis uma revisao criteriosa da skill praxis-manual-form-autosave-persistence para garantir que ela esteja 100% em conformidade com os recursos disponiveis do Praxis e com as melhores praticas de uso desses recursos.
...
- Familia: praxis
- Caminho: codex-skills/praxis-manual-form-autosave-persistence/
- Descricao atual: Use when implementing or auditing `@praxisui/manual-form` autosave, draft persistence, `ManualFormInstance.saveDraft()`, `resetToSeed()`, storage key composition, `ManualFormPersistenceOptions`, `ASYNC_CONFIG_STORAGE`, SSR-safe storage, debounce, or persisted config/value reload.

# Revisar skill praxis: praxis-table-renderer-state-diagnostics

## Objetivo

Solicitar a um especialista em plataforma Praxis uma revisao criteriosa da skill praxis-table-renderer-state-diagnostics para garantir que ela esteja 100% em conformidade com os recursos disponiveis do Praxis e com as melhores praticas de uso desses recursos.
...
- Familia: praxis
- Caminho: codex-skills/praxis-table-renderer-state-diagnostics/
- Descricao atual: Use when Codex must implement, audit, or debug @praxisui/table renderers and user-visible states: column renderers, rich-content adapters, data formatting, empty/loading/error states, loadingStateChange, runtime observations, diagnostics, i18n copy, row/column renderer precedence, or renderer-specific specs.

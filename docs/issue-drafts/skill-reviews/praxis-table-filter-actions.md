# Revisar skill praxis: praxis-table-filter-actions

## Objetivo

Solicitar a um especialista em plataforma Praxis uma revisao criteriosa da skill praxis-table-filter-actions para garantir que ela esteja 100% em conformidade com os recursos disponiveis do Praxis e com as melhores praticas de uso desses recursos.
...
- Familia: praxis
- Caminho: codex-skills/praxis-table-filter-actions/
- Descricao atual: Use when Codex must work on @praxisui/table filters, dynamic filter payloads, filter settings, filter drawer adapter, inline filters, toolbar actions, row actions, bulk actions, global actions, CRUD actions, export operations, or action/surface capability grounding.

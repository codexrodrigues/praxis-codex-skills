# Revisar skill praxis: praxis-manual-form-toolbar-metadata-bridge

## Objetivo

Solicitar a um especialista em plataforma Praxis uma revisao criteriosa da skill praxis-manual-form-toolbar-metadata-bridge para garantir que ela esteja 100% em conformidade com os recursos disponiveis do Praxis e com as melhores praticas de uso desses recursos.
...
- Familia: praxis
- Caminho: codex-skills/praxis-manual-form-toolbar-metadata-bridge/
- Descricao atual: Use when implementing or auditing `@praxisui/manual-form` field customization UI, floating toolbar, `enableCustomization`, required/readOnly/hidden/disabled toggles, `pdxManualEdit`, `tryOpenFieldEditor()`, `ManualFieldMetadataBridgeService`, Settings Panel integration, lazy `@praxisui/metadata-editor` loading, or metadata patch diagnostics.

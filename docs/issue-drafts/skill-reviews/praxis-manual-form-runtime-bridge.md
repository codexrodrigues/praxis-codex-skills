# Revisar skill praxis: praxis-manual-form-runtime-bridge

## Objetivo

Solicitar a um especialista em plataforma Praxis uma revisao criteriosa da skill praxis-manual-form-runtime-bridge para garantir que ela esteja 100% em conformidade com os recursos disponiveis do Praxis e com as melhores praticas de uso desses recursos.
...
- Familia: praxis
- Caminho: codex-skills/praxis-manual-form-runtime-bridge/
- Descricao atual: Use when implementing or auditing `@praxisui/manual-form` runtime behavior, including declarative manual forms with `pdx-*` projected fields, host `FormGroup` adoption, field autodetection, `ManualFormInstance`, autosave/persistence through `ASYNC_CONFIG_STORAGE`, `ManualFieldKeyService`, field toolbar, `pdxManualEdit`, `ManualFieldMetadataBridgeService`, metadata-editor lazy bridge, and dynamic-fields/core integration.

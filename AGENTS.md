...
## Coordenação e revisão da plataforma

Para trabalho Praxis, consultar [docs/platform-governance/README.md](docs/platform-governance/README.md) ao delegar ou assumir uma execução prolongada. A política exige modelo/esforço justificados, revisão abrangente independente de implementação, avaliação de skills e continuidade rastreável. Não presume instalação automática no aplicativo nem substitui as regras locais de validação e publicação deste repositório.
